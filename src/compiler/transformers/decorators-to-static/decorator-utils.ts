...
const getDecoratorParameterWithConstants = (arg: ts.Expression, typeChecker: ts.TypeChecker): any => {
  if (ts.isObjectLiteralExpression(arg)) {
    // Enhanced: Pass type checker to support constant resolution in object literals
...
export const getDecoratorParametersWithConstants: GetDecoratorParameters = (
  ...
  return decorator.expression.arguments.map((arg) => getDecoratorParameterWithConstants(arg, typeChecker));
};
