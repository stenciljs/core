...
describe('lazy-load failure recovery', () => {
  ...
  beforeEach(() => {
    ...
    // The retry backoff (connected-callback.ts) is driven by a plain global
    // `setTimeout`. Rather than pulling in fake-timer machinery (which
    // conflicts with this project's custom Jest environment and the
    // `process.nextTick`-driven testing task queue), the retry callback is
    // invoked synchronously here so reconnect-triggered retries can be
    // asserted deterministically. The delay itself is still verified via
  ...
  it(`gives up after ${MAX_LAZY_LOAD_RETRIES} failed attempts and stops retrying on further reconnects`, async () => {
    ...
    for (let attempt = 1; attempt < MAX_LAZY_LOAD_RETRIES; attempt++) {
      expect((getHostRef(elm)?.$flags$ ?? 0) & HOST_FLAGS.hasFailedLoad).toBe(HOST_FLAGS.hasFailedLoad);
      elm.remove();
      win.document.body.appendChild(elm);
      await flushLoadModule(bundleId);
      await flushAll().catch(() => {});
