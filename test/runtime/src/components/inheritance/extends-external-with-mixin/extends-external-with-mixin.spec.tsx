...
/**
 * Tests for extending from external library components that use mixin patterns.
 * Verifies that decorated members from components with internal mixin patterns
 * in sibling project are properly inherited and reactive (Bug A fix).
 */
describe('extends-external-with-mixin', () => {
  describe('extending from external component with mixin pattern', () => {
    it('renders default values', async () => {
      const { root } = await render(<extends-external-with-mixin />);
    ...
    it('re-renders values via attributes', async () => {
      const { root, waitForChanges } = await render(<extends-external-with-mixin />);
      ...
      expect(root.querySelector('.main-prop-2')).toHaveTextContent('Main class prop2: external via attribute');
    });
    ...
    it('re-renders values via props', async () => {
      ...
      expect(root.querySelector('.main-prop-2')).toHaveTextContent('Main class prop2: external via prop');
    });
