...
/**
 * Smoke tests for extending mixin classes.
 *  `tsconfig.json` > `"target": "es2022"` `dist` and `dist-custom-elements` outputs.
 */

describe('Checks component classes can extend from other mixin classes', () => {
  describe('es2022 dist output', () => {
    ...
    it('renders default values', async () => {
      const { defaultValue } = await testSuites(frameContent, 'extends-mixin');
      await defaultValue();
    });

    it('re-renders values via attributes', async () => {
      const { viaAttributes } = await testSuites(frameContent, 'extends-mixin');
      await viaAttributes();
    });

    it('re-renders values via props', async () => {
      const { viaProps } = await testSuites(frameContent, 'extends-mixin');
      await viaProps();
    });

    it('calls watch handlers', async () => {
      const { watchHandlers } = await testSuites(frameContent, 'extends-mixin');
      await watchHandlers();
    });

    it('calls methods', async () => {
      const { methods } = await testSuites(frameContent, 'extends-mixin');
      await methods();
    });
  });
  ...
  describe('hydrate output', () => {
    it('renders component during SSR hydration via attributes', async () => {
      ...
      await (await testSuites(document.body, 'extends-mixin')).ssrViaAttrs(mod);
    });

    it('renders component during SSR hydration via props', async () => {
      // @ts-ignore may not be existing when project hasn't been built
      const mod = await import('/test-ts-target-output/hydrate/index.mjs');
      await (await testSuites(document.body, 'extends-mixin')).ssrViaProps(mod);
    });
  });
});
