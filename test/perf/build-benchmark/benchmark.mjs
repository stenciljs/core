 ...
 *
 * Measures cold (no cache) and warm (with cache) build times.
 * Uses integration/e2e as the benchmark fixture.
 */
...
const COLD_RUNS = 5;
const WARM_RUNS = 5;
const E2E_DIR = path.join(__dirname, '..', '..', 'integration', 'e2e');
const CACHE_DIR = path.join(E2E_DIR, '.stencil');
...
function runBuild() {
  ...
  const result = spawnSync('npx', ['stencil', 'build', '--config', './stencil.build.config.ts'], {
    cwd: E2E_DIR,
    stdio: 'pipe',
    encoding: 'utf-8',
    shell: true,
...
async function main() {
  ...
  console.log('===================================');
  console.log(`Fixture: integration/e2e`);
