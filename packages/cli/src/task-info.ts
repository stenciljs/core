import type { CompilerSystem, Logger } from '@stencil/core/compiler';
...
export const taskInfo = (coreCompiler: CoreCompiler, sys: CompilerSystem, logger: Logger): void => {
  ...
  console.log(
    `${logger.cyan('     Stencil:')} ${coreCompiler.version}${logger.emoji(' ' + coreCompiler.vermoji)}`,
  );
  console.log(`${logger.cyan('  TypeScript:')} ${versions.typescript}`);
  console.log(`${logger.cyan('      Rollup:')} ${versions.rolldown}`);
