...
import { STENCIL_APP_DATA_ID } from '../bundle/entry-alias-ids';
...
export const transpileModule = (
  ...
  const compilerHost: ts.CompilerHost = {
    getSourceFile: (fileName) =>
      normalizePath(fileName) === normalizePath(sourceFilePath) ? sourceFile : undefined,
    writeFile: (name, text) => {
  ...
  if (transformOpts.buildOverrides && results.code) {
    const mutations = Object.entries(transformOpts.buildOverrides)
      .filter(([, v]) => v !== undefined)
      .map(([k, v]) => `__stencilBUILD.${k}=${v};`)
      .join('');
    if (mutations) {
      results.code =
        `import{BUILD as __stencilBUILD}from"${STENCIL_APP_DATA_ID}";\n${mutations}\n` +
        results.code;
    }
  }
