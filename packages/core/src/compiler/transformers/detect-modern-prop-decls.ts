 ...
 * @param sourceFile the TypeScript source file containing the class
...
export const detectModernPropDeclarations = (
  ...
  const parsedStates: { [key: string]: d.ComponentCompilerProperty } = getStaticValue(
    classNode.members,
    'states',
  );
    ...
    const prop = classNode.members.find((m) => {
      return (
        ...
        ((ts.isComputedPropertyName(m.name) &&
          m.name.expression.getText(sourceFile) === dynamicPropName) ||
