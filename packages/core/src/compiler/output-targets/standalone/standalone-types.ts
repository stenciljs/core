...
const generateStandaloneTypesOutput = async (
  ...
  // Relative path from standalone dir to types/standalone.d.ts
  const standaloneTypesRelPath = relDts(outputTarget.dir!, join(typesDir, 'standalone.d.ts'));
  ...
  const code = [
    ...
    ...(isBarrelExport
      ...
      : []),
    // Re-export standalone API types from the centralized types directory
    `export * from '${standaloneTypesRelPath}';`,
