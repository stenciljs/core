...
describe('optimize-module', () => {
  describe('prepareModule', () => {
    ...
    // bundler directive comments being present in the minified output
  ...
  return import(
    ...
    it('preserves bundler directive comments inside dynamic imports', async () => {
      const config = mockValidatedConfig({ logLevel: 'info', sourceMap: false });
      const minifyOpts = getTerserOptions(config, 'es2017', false);

      const results = await prepareModule(loadModuleSource, minifyOpts);
    ...
    it('still removes regular comments', async () => {
      const config = mockValidatedConfig({ logLevel: 'info', sourceMap: false });
      const minifyOpts = getTerserOptions(config, 'es2017', false);
...
export const add = (a, b) => a + b;`;
      ...
      const results = await prepareModule(input, minifyOpts);
    ...
    it('preserves license comments', async () => {
      const config = mockValidatedConfig({ logLevel: 'info', sourceMap: false });
      const minifyOpts = getTerserOptions(config, 'es2017', false);
