...
describe('vdom signal bypass — <Show>', () => {
  it('renders children visible when signal is initially true', async () => {
    ...
    const { root } = await newSpecPage({ ...VDOM, components: [CmpA], html: `<cmp-a></cmp-a>` });
    const wrapper = root.querySelector('s-show') as HTMLElement;
    expect(wrapper).toBeTruthy();
    expect(wrapper.style.display).toBe('contents');
    expect(wrapper.querySelector('span').textContent).toBe('content');
  });

  it('renders children hidden when signal is initially false', async () => {
  ...
  it('toggles display without re-rendering the component', async () => {
    let renderCount = 0;
    const visible = signal(true);
  ...
  it('renders children statically when `when` is a plain boolean (true)', async () => {
    @Component({ tag: 'cmp-a' })
    class CmpA {
      render() {
        return (
    ...
    expect(root.querySelector('s-show')).toBeNull();
  });

  it('renders nothing when `when` is a plain boolean (false)', async () => {
    @Component({ tag: 'cmp-a' })
    class CmpA {
      render() {
        return (
