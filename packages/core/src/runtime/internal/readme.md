...
By default, when Stencil resolves `@stencil/core/runtime`, it's going to assume it wants the `client` internals (rather than `hydrate`). So by default, `@stencil/core/runtime` actually points to `@stencil/core/runtime/client`.
