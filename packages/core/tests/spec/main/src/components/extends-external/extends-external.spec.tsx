...
/**
 * Tests for extending from external library component classes.
 * Verifies that decorated members from sibling project components
 * are properly inherited and reactive.
 */
describe('extends-external', () => {
  describe('extending from external library component', () => {
    it('renders default values', async () => {
      const { root } = await render(<extends-external />);
    ...
    it('re-renders values via attributes', async () => {
      const { root, waitForChanges } = await render(<extends-external />);
