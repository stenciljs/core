...
describe('scoped-slot-slotchange', () => {
  ...
  it('checks that external, browser content changes fire slotchange events', async () => {
    ...
    expect(slotChangeEle.slotEventCatch[0].assignedNodes[0].outerHTML).toMatch(`<p>Append child content</p>`);
    ...
    expect(slotChangeEle.slotEventCatch[2].assignedNodes[1].outerHTML).toMatch(`<div>InsertBefore content</div>`);
  });
});
