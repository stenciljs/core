...
describe('transformStencil', () => {
  it('returns null for non-tsx/ts files', async () => {
    expect(await transformStencil('const x = 1;', '/src/foo.js', {})).toBeNull();
  });

  it('returns null for .d.ts files', async () => {
    expect(await transformStencil('export type Foo = {};', '/src/foo.d.ts', {})).toBeNull();
  });

  it('transforms a Stencil component', async () => {
    const result = await transformStencil(COMPONENT, '/src/my-button.tsx', {});
  ...
  it('rewrites CSS imports with queryparams', async () => {
    ...
    `;
    const result = await transformStencil(cmpWithStyle, '/src/styled-cmp.tsx', {});
    expect(result).not.toBeNull();
    // styleImportData: 'queryparams' appends ?tag=... to the CSS import
    expect(result!.code).toMatch(/styled-cmp\.css\?/);
  });

  it('returns null for a plain TS file with no @Component', async () => {
  ...
  it('returns tagName in result', async () => {
    const result = await transformStencil(COMPONENT, '/src/my-button.tsx', {});
    expect(result!.tagName).toBe('my-button');
  });

  describe('dev mode HMR injection', () => {
    it('injects nothing when dev=false', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', {}, false);
      expect(result!.code).not.toContain('import.meta.hot');
      expect(result!.code).not.toContain('__stencil_module__');
    });

    it('injects Vite HMR snippet for vite framework', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', {}, true, 'vite');
    ...
    it('injects Vite HMR snippet as default when framework is unrecognised', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', {}, true, 'rollup');
      expect(result!.code).toContain("import.meta.hot.on('stencil:hmr'");
    });

    it('injects webpack HMR snippet for webpack framework', async () => {
    ...
    it('injects webpack HMR snippet for rspack framework', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', {}, true, 'rspack');
      expect(result!.code).toContain('module.hot');
      expect(result!.code).toContain('import.meta.webpackHot');
      expect(result!.code).not.toContain("import.meta.hot.on('stencil:hmr'");
    });

    it('injects bun HMR snippet using import.meta.hot with dispose/accept/data', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', {}, true, 'bun');
      expect(result!.code).toContain('__stencil_module__ = import.meta.url');
  ...
  describe('mode: spec-page', () => {
    it('emits COMPILER_META instead of a self-registering custom element', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', { mode: 'spec-page' });
      expect(result).not.toBeNull();
    ...
    it('imports from @stencil/core/testing rather than the standalone client', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', { mode: 'spec-page' });
      expect(result!.code).toContain('@stencil/core/testing');
    });

    it('does not double-export the class (compilerstatic keeps the source export)', async () => {
      const result = await transformStencil(COMPONENT, '/src/my-button.tsx', { mode: 'spec-page' });
      expect(result!.code).toMatch(/export const MyButton/);
      expect(result!.code).not.toContain('export{MyButton}');
    });

    it('injects no HMR snippet even when dev=true', async () => {
      const result = await transformStencil(
        COMPONENT,
        '/src/my-button.tsx',
        { mode: 'spec-page' },
    ...
    it('does not emit an unresolved styleUrl import (no CSS loader needed for spec-page tests)', async () => {
      const cmpWithStyle = `
        import { Component, h } from '@stencil/core';
        @Component({ tag: 'styled-cmp', styleUrl: 'styled-cmp.css', encapsulation: { type: 'shadow' } })
        export class StyledCmp { render() { return <div />; } }
