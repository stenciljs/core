...
describe('isHtmlFile', () => {
  ...
  it.each(['.ht', 'foo.htmx', 'foo/bar.xaml'])(
    'returns false for other types of files (%s)',
    (filename) => {
  ...
  it.each(['.hTMl', 'foo.HTML', 'foo/bar.htmL'])(
    'is case insensitive for filename (%s)',
    (filename) => {
      expect(isHtmlFile(filename)).toBe(true);
    },
  );
...
describe('isCssFile', () => {
  ...
  it.each(['.txt', 'foo.sass', 'foo/bar.htm'])(
    'returns false for other types of files (%s)',
    (filename) => {
  ...
  it.each(['.cSs', 'foo.cSS', 'foo/bar.CSS'])(
    'is case insensitive for filename (%s)',
    (filename) => {
      expect(isCssFile(filename)).toBe(true);
    },
