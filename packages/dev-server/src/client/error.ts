...
import appErrorCss from './error.css' with { type: 'css' };
...
export const appError = (data: AppErrorData): AppErrorResults => {
  ...
  if (data && data.window && Array.isArray(data.buildResults.diagnostics)) {
    const diagnostics = (data.buildResults as CompilerBuildResults).diagnostics.filter(
      (diagnostic: Diagnostic) => diagnostic.level === 'error',
    );
...
const closeDevServerModal = (doc: Document) => {
...
export const clearAppErrorModal = (data: { window: Window }) => {
  ...
  if (appErrorElm) {
    const closeOnEsc = (appErrorElm as any).__closeOnEsc;
    if (closeOnEsc) {
      data.window.document.removeEventListener('keydown', closeOnEsc);
    }
...
const removeErrorBadge = (doc: Document) => {
