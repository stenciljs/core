...
export const hmrExternalStyles = (
  elm: Element,
  versionId: string,
  cssFileNames: string[],
): string[] => {
  ...
  if (isTemplate(elm)) {
    hmrExternalStyles(
      (elm as HTMLTemplateElement).content as unknown as Element,
      versionId,
      cssFileNames,
    );
  }
...
const hmrStylesheetLink = (
  styleSheetElm: HTMLLinkElement,
  versionId: string,
  cssFileName: string,
): void => {
...
export const hmrInlineStyles = (
  elm: Element,
  versionId: string,
  stylesUpdatedData: HmrStyleUpdate[],
): string[] => {
...
const findComponentInstancesTraverse = (
  elm: Element,
  tagName: string,
  instances: Element[],
): void => {
...
const createStyleElement = (
  ...
  const doc =
    'ownerDocument' in container
      ...
      : (container as ShadowRoot).ownerDocument;
